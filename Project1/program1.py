# At least one of those programs should get input from the user, 
# and then construct some output that uses that input as part of it. 
# At least one of those programs should do some math. Project Examples/Ideas
#  on a separate page. Your project does not have to follow these examples 
# precisely (you can make your own thing, if you really want) but if you're 
# struggling to come up with an idea I would suggest you follow the examples.


# GROUP SESSION:
# print("This program converts Japanese Yen to Phillipine Peso.")
# print("1 yen = 0.41 peso ")
# yen=input("How many yen are you converting? ")
# peso =float(yen)*0.41
# print(f"{yen} yen is {peso} pesos")


print("1 kilogram = 2.20 pounds")
kilogram=input("How many kilograms are you converting? ")
# In order for the program to execute properly, the input needs to be converted 
# from a string into a number
pounds=float(kilogram)*2.20
print(f"{kilogram} kilograms is {pounds} pounds")